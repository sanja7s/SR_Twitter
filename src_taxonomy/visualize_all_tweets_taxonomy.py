...
# -*- coding: UTF-8 -*-
'''
	plot me hostograms of the top concepts, entities, taxonomies etc per community
'''
...
def plot_taxonomy_pies(COM="ALL", user_list=None, TOP_N=10):
	...
	with codecs.open(f_in,'r', encoding='utf8') as input_file:
		...
		for el in ord_keywords_sum2:
			...
			labels.append(el)
			sizes.append(float(ord_keywords_sum2[el]))
			i += 1
			if i == TOP_N:
				break
		plot_pie(labels, sizes, "kw_com_" + str(COM) + "_top_" + str(TOP_N) + ".png")
		print
...
def main(COM='ALL'):
	...
	if COM == "ALL":
		plot_taxonomy_pies("ALL")
	else:
		for community in top_communities:
			plot_taxonomy_pies(str(community), top_communities[community])
