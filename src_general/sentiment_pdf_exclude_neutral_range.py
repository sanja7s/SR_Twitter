...
def read_in_sent(min_threshold = -0.1, max_threshold = 0.1):
	...
	for line in f:
		...
		if float(sent) < min_threshold or float(sent) > max_threshold:
...
def read_in_edges_sent(f2, threshold = 0, min_threshold = -0.1, max_threshold = 0.1):
	...
	for line in f2:
		...
		w = int(w)
		if w > threshold:
			if (sent_dat[uid1] < min_threshold or sent_dat[uid1] > max_threshold) and uid1 != uid2 and \
					(sent_dat[uid2] < min_threshold or sent_dat[uid2] > max_threshold):
...
def plot_pdf(ydata):
	...
	plt.savefig("sentiment/neutral_range_out/neutral_range_out_histogram_user_sent.eps", dpi = 440)
	#plt.show()
...
def plot_pdf_line(ydata):
	...
	plt.savefig("sentiment/neutral_range_out/neutral_range_out__user_sent.pdf", dpi = 200)
...
def plot_cum_distr(ydata):
	...
	# Tweak spacing to prevent clipping of ylabel
	plt.subplots_adjust(left=0.15)
	plt.savefig("sentiment/neutral_range_out/neutral_range_out_user_sent_cum.eps", dpi = 440)
	#plt.show()
...
def scatterplot_edges_sent_diff_v2(edges_sent, edges_sent_weighted):
	...
	heatmap, extent = get_heatmap(edges_sent)
	fig_name = "neutral_range_out_one_sided.png"
	plot_heatmap(heatmap, extent, fig_name, colmap=cm.RdGy)

	heatmap_w, extent_w = get_heatmap(edges_sent_weighted)
	fig_name_w = "neutral_range_out_one_sided_weighted.png"
