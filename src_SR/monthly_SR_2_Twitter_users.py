...
def save_monthly_SR_2_users(usrA, usrB):

	f = open("monthly_SR_change_2_usr/monthly_SR_change_" + usrA + "_" + usrB, "w")

	for MO in MONTHS.keys():
		print MO
		SR, num_tweetsA, num_tweetsB = MO_SR_2_users(usrA, usrB, MO)
		print SR, num_tweetsA, num_tweetsB
		f.write(str(MO) + '\t' + str(SR) + '\t' + str(num_tweetsA) + '\t' + str(num_tweetsB) + '\n')
...
# These are examples for which we have plotted the interaction from before

#save_monthly_SR_2_users("garywright13", "DarrenLA")
#save_monthly_SR_2_users("jucksonline", "Saydoh")

#save_monthly_SR_2_users("KeithHagel", "WayneHagel")

#save_monthly_SR_2_users("pgnimmo", "tcl189")

#save_monthly_SR_2_users("siledubh", "kismit1496")

#save_monthly_SR_2_users("xBon_Bonx", "Elisha_Ro_Tii")
