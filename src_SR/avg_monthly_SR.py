...
def read_in_all_monthly_SR_SR_NETW():
	monthly_SR = defaultdict(dict)
	for MO in MONTHS:
		for s in parts:
			f_in_SR = MO + "MOSR/" + str(MO) + "_all_user_SR_part_" + s
			f = open(f_in_SR, 'r')
			for line in f:
				(u1, u2, SR) = line.split()
				userA = int(u1)
				...
				if monthly_SR[(u1, u2)][MO] == 0:
					...
					assert monthly_SR[(u1, u2)][MO] == SR
			f.close()
		return monthly_SR

def read_in_all_monthly_SR_MENT():
...
def calculate_monthly_avg_SR_no_self_loops():
	monthly_SR = read_in_all_monthly_SR_MENT()
...
def calculate_monthly_avg_SR_SR_NETW():
	monthly_SR = read_in_all_monthly_SR_SR_NETW()
	avg_May = []
	avg_June = []
	avg_July = []
	avg_Aug = []
...
calculate_monthly_avg_SR_SR_NETW()
