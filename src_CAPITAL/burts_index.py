...
def read_networkx_graph():
	G = nx.read_edgelist(f_in_graph_weights,create_using=nx.DiGraph(),nodetype = int, data=(('weight',float),))
	return G

G = read_networkx_graph()

BI = structural_holes(G)
print BI
f = open('BI_all', 'w')
f2 = open('BI', 'w')
for el in BI:
	#print el
	f.write(str(el) + '\t' + str(BI[el]) + '\n')
	#f2.write(str(el) + '\t' + str(BI[el]['C-Index']) + '\n')